...
from utils_regularized import BNFeatureHook, lr_cosine_policy,DiversityLoss,save_images_multiple
...
def get_images(args, model_teacher, hook_for_display):
    original_embeddings = {}
    #load the embeddings
    for class_id in range(NUM_CLASSES):
        original_embeddings[class_id] = torch.load("embeddings/class_{}.pt".format(class_id),map_location=args.device)
    ...
    for class_id in range(args.start_class_id,args.start_class_id+batch_size,batch_size):
        targets = torch.LongTensor(np.repeat(np.arange(class_id,class_id+batch_size),args.IPC)).to(args.device)

        for class_idx in range(args.start_class_id,args.start_class_id+batch_size):
            loaded_tensor = torch.load(args.init_path+'tensor_{}.pt'.format(class_idx),map_location=args.device)
            if class_idx == args.start_class_id:
                inputs = loaded_tensor[args.ipc_start:args.ipc_start+args.IPC,:,:,:].detach().clone()
            else:
                inputs = torch.vstack((inputs,loaded_tensor[args.ipc_start:args.ipc_start+args.IPC,:,:,:].detach().clone()))
        print(inputs.shape)
        ...
        criterion_reg = DiversityLoss(args,batch_size)
        

        for iteration in tqdm(range(iterations_per_layer)):
            ...
            synthetic_loss = criterion_reg(activation["avgpool"].reshape(-1,512),original_embeddings)
            ...
            loss = loss_ce + loss_aux + synthetic_loss 
        ...
        if args.store_best_images:
            ...
            save_images_multiple(args, best_inputs, targets,args.IPC)
...
def main_syn(args):
    ...
    checkpoint = torch.load(args.arch_path,map_location=args.device)
    ...
    get_images(args, model_teacher, hook_for_display)
...
def parse_args():
    ...
    parser.add_argument("--multiplier",type=float,default=0.01)
    parser.add_argument('--init-path', default='./init_images/cifar100/', type=str)
    parser.add_argument("--r_cos",type=float,default=1.0)
    parser.add_argument("--r_euc",type=float,default=0.01)
...
if __name__ == "__main__":
    ...
    print("=========================================")
    print("[INFO] Settings...")
    print("[INFO] Starting class:{}, Ending class:{}, IPC: {}, images per class:{}".format(args.start_class_id,args.start_class_id+args.batch_size,args.ipc_start,args.IPC))
    print("=========================================")
    main_syn(args)
